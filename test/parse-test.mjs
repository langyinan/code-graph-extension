...
for (const file of files) {
  ...
  console.log(`\n=== ${file} ===`);
  console.log(`  imports (${imports.length}): ${imports.length ? imports.join(', ') : '—'}`);
  console.log(`  calls   (${calls.length}): ${calls.length ? calls.map(c => `${c.caller}→${c.callee}`).join(', ') : '—'}`);
}

console.log(`\n${files.length} fixtures · ${imported} produced imports · ${withCalls} produced call edges`);
